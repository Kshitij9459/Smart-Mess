...
import os
from flaskblog.models import User, Post, Complaint, Menu, NewComplaint
from flask import Flask, render_template, url_for, flash, redirect, request,abort
from flaskblog.form import RegistrationForm, LoginForm, UpdateAccountForm, PostForm, MenuForm, NewMenuForm, FindDate
...
with open('flaskblog/vect.pkl','rb') as f:
    vectorizer=pickle.load(f)
with open('flaskblog/funny.pkl','rb') as f:
...
def encode_docs(tokenizer, max_length, docs):
    ...
    return padded
Tokenizer =pickle.load(open("flaskblog/TOKENIZER.pickle", "rb"))
'''vectorizer = TfidfVectorizer(min_df=4, max_df=0.8,
                            sublinear_tf=True, use_idf=True)'''
classifier_liblinear = pickle.load(open('flaskblog/funny.pkl','rb'))
sentiment = pickle.load(open('flaskblog/Sentiment_Model.pickle','rb'))
@app.route("/")
@app.route("/home")
def home():
	data = Menu.query.all()
...
def display_complaints():
	...
	if form.validate_on_submit():
		...
		wc.to_file('flaskblog/static/img/'+random_hex+'.png')
